...
const loadContextData = async () => {
    const contextResponse = await fetch("/static/data/contexts/codemeta-2.0.jsonld");
...
async function generateCodemeta() {
    ...
    if (inputForm.checkValidity()) {
        ...
        // Inicializo el boton de salvar


        document.querySelector('#saveCodemetaToFile').disabled = false;
        document.querySelector('#saveCodemetaToFile')
            .addEventListener('click', saveCodemetaToFile);
...
async function saveCodemetaToFile() {
    var codemetaText = document.querySelector('#codemetaText').innerText;
    if (codemetaText) {
        saveJSONToFile(codemetaText);
        alert('The code has been saved successfully.');
    } else {
        alert('No codemeta text available to save.');
    }
}

function saveJSONToFile(jsonData) {
    const blob = new Blob([jsonData], { type: 'application/json' });
    const fileName = 'codemeta_saved.json';
